    ...
    prepare(sql: string) {
      ...
      return {
        run: (...args: any[]) => {
          ...
          if (normalized.startsWith('UPDATE refresh_tokens SET revoked_at = CURRENT_TIMESTAMP, replaced_by_token_hash = COALESCE')) {
            ...
            for (const row of state.refreshTokens) {
              if (row.token_hash === tokenHash && row.revoked_at === null) {
                ...
                row.replaced_by_token_hash = replacementHash ?? row.replaced_by_token_hash;
                changes++;
              }
            }
            return { changes };
          ...
          if (normalized.startsWith('UPDATE refresh_tokens SET revoked_at = CURRENT_TIMESTAMP WHERE user_id = ?')) {
            const [userId] = args;
            let changes = 0;
            for (const row of state.refreshTokens) {
        ...
        get: (...args: any[]) => {
          ...
          if (normalized.startsWith('SELECT user_id FROM refresh_tokens WHERE token_hash = ?')) {
            const tokenHash = args[0];
            const row = state.refreshTokens.find((r) => r.token_hash === tokenHash && r.revoked_at === null);
...
let authRouter: (typeof import('../routes/auth.js'))['default'];
...
function getCookie(headers: string[] | string | undefined, cookieName: string) {
...
async function registerDefaultUser(app: express.Express) {
...
describe('Auth refresh token security', () => {
  it('rotates refresh token and rejects old token reuse', async () => {
    ...
    const oldRefresh = getCookie(registerRes.headers['set-cookie'], 'refresh_token');
    ...
    const newRefresh = getCookie(refreshRes.headers['set-cookie'], 'refresh_token');
  ...
  it('revokes refresh token on logout', async () => {
    const app = makeApp();
    const registerRes = await registerDefaultUser(app);
    const refreshCookie = getCookie(registerRes.headers['set-cookie'], 'refresh_token');
  ...
  it('revokes all active sessions after password change', async () => {
    ...
    const sessionACookies = registerRes.headers['set-cookie'] as string[] | string | undefined;
