...
export type ProcessState = 'pending' | 'hired' | 'in_progress' | 'review' | 'completed';
...
const STAGES = [
  { id: 'pending', label: 'Pending', icon: Clock, description: 'Finding workers' },
  { id: 'hired', label: 'Hired', icon: UserCheck, description: 'Provider selected' },
  { id: 'in_progress', label: 'In Progress', icon: Play, description: 'Work being done' },
  { id: 'review', label: 'Review & Pay', icon: CreditCard, description: 'Finalize task' },
];

export const ProcessTimeline: React.FC<ProcessTimelineProps> = ({ currentState, className = '' }) => {
          ...
          return (
            <div key={stage.id} className="relative z-10 flex flex-col items-center group">
              ...
              <div className="mt-3 text-center">
                <p className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${isActive ? 'text-brand-primary' : isCompleted ? 'text-brand-text-main' : 'text-brand-text-variant'}`}>
                  {stage.label}
                </p>
                <p className={`text-[9px] font-medium hidden md:block transition-opacity ${isActive ? 'text-brand-text-variant opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                  {stage.description}
                </p>
              </div>
