...
const HelpCard = ({ icon: Icon, title, description, children }) => (
...
const FAQItem = ({ question, answer }) => (
  <div className="group cursor-pointer">
    <div className="flex justify-between items-start gap-4 mb-2">
      ...
      <ChevronRight
        size={14}
        className="text-brand-text-variant mt-1 shrink-0 group-hover:translate-x-1 transition-transform"
      />
    </div>
    <p className="text-xs text-brand-text-variant leading-relaxed">{answer}</p>
  </div>
);
    ...
    <div className="max-w-6xl mx-auto py-12 px-8 lg:px-12">
      <header className="mb-12 text-center">
        ...
        <p className="text-brand-text-variant max-w-2xl mx-auto">
          Need assistance with your tasks or payments? Find answers to commonly
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <button
              ...
              className="px-8 py-3.5 bg-white border-2 border-brand-outline text-[#0f172a] font-bold rounded-xl hover:text-brand-primary hover:border-brand-primary/50 transition-all flex items-center gap-2"
            >
        ...
        {isContactOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-brand-surface rounded-[2.5rem] shadow-2xl border border-brand-outline p-8"
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  onClick={() => setIsContactOpen(false)}
                  className="p-2 hover:bg-brand-outline/30 rounded-full transition-colors"
                >
                  <X size={20} />
                </button>
              </div>
              <form onSubmit={handleContactSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-brand-text-main uppercase tracking-widest mb-2">
                    Subject
                  </label>
                  <input
                    required
                    ...
                    className="w-full px-4 py-3 bg-brand-surface border-2 border-brand-outline rounded-xl focus:border-brand-primary outline-none transition-all"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-brand-text-main uppercase tracking-widest mb-2">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-brand-surface border-2 border-brand-outline rounded-xl focus:border-brand-primary outline-none transition-all resize-none"
                  />
                </div>
                <button
                  disabled={isSubmitting}
                  type="submit"
                  className="w-full py-4 bg-brand-primary text-white font-bold rounded-xl shadow-lg hover:shadow-brand-primary/20 transition-all flex items-center justify-center gap-2"
                >
                  {isSubmitting ? (
