...
import { X, MapPin, Banknote, Star, ShieldCheck } from "lucide-react";
import { api } from "../../../services/api";

export const UserProfileModal = ({ userId, isOpen, onClose }) => {
      ...
      try {
        const [userData, userReviews] = await Promise.all([
          ...
          api
            ...
            .catch(() => ({ reviews: [], avg_rating: 0, total_reviews: 0 })),
        ]);
        setUser(userData);
      ...
      <div className="relative bg-brand-surface-card w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col text-left max-h-[90vh] border border-white/10 transition-all duration-300">
        ...
        {isLoading ? (
          <div className="py-24 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-brand-text-variant text-xs font-black uppercase tracking-widest animate-pulse">
              Loading Profile...
            </p>
          </div>
        ) : error || !user ? (
          ...
          <>
            ...
            <div className="p-8 overflow-y-auto bg-brand-surface/30 space-y-6 flex-1">
              ...
              <div>
                ...
                <div className="bg-brand-surface-card p-5 rounded-2xl border border-brand-outline shadow-inner">
                  <p className="text-sm text-brand-text-main italic leading-relaxed">
                    "
                    {user.about_me ||
                      "This client is active in hiring skilled neighborhood professionals for household and general service tasks."}
                    "
                  </p>
                </div>
              </div>
                ...
                <div className="bg-brand-surface-card/65 p-4 rounded-2xl border border-brand-outline flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-bold text-brand-text-main truncate">
                      {user.location || "Dasmariñas, Cavite"}
                    </p>
                  </div>
                </div>
              ...
              {/* Reviews Feed */}
              <div>
                <h4 className="text-xs font-black uppercase tracking-[0.15em] text-brand-text-variant mb-3">
                  Reviews from Service Providers
                </h4>
                {reviewsData.reviews && reviewsData.reviews.length > 0 ? (
                  <div className="space-y-4">
                    {reviewsData.reviews.map((rev) => (
                      <div
                        key={rev.id}
                        className="bg-brand-surface-card border border-brand-outline p-5 rounded-2xl space-y-3"
                      >
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex items-center gap-3">
                            {rev.reviewer_avatar ? (
                              <img
                                src={
                                  rev.reviewer_avatar.startsWith("http")
                                    ? rev.reviewer_avatar
                                    : `http://localhost:3000${rev.reviewer_avatar}`
                                }
                            ...
                            <div>
                              <p className="text-xs font-bold text-brand-text-main">
                                {rev.reviewer_name || "Service Provider"}
                              </p>
                              <p className="text-[9px] text-brand-text-variant mt-0.5">
                                Reviewed on task:{" "}
                        ...
                        <p className="text-xs text-brand-text-variant italic leading-relaxed pl-1">
                          "
                          {rev.comment ||
                            "Smooth communication, straightforward task, and prompt arrival verification. Highly recommended client!"}
                          "
                        </p>
                      </div>
                    ))}
