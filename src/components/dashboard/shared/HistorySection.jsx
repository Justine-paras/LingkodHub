...
export const HistorySection = () => {
  ...
  const fetchHistory = React.useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        reviews = Array.isArray(reviewData)
          ? reviewData
          : reviewData.reviews || [];
      } catch (e) {
      ...
      const mapped = allJobs.map((job) => {
        ...
        return {
          ...job,
          date: new Date(job.created_at).toLocaleDateString(undefined, {
            month: "short",
            day: "2-digit",
            year: "numeric",
          }),
          ...
          workerAvatar:
            me.role === "provider" ? job.client_avatar : job.provider_avatar,
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedIds.length} history records?`,
      )
    )
      return;
    try {
    ...
    <div className="max-w-6xl mx-auto py-10 px-8 w-full flex flex-col h-[calc(100vh-80px)]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 shrink-0">
        <div>
          ...
          <p className="text-sm text-brand-text-variant font-medium">
            Manage your past transactions and job records.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 shrink-0">
        <div className="bg-brand-surface-card border border-brand-outline rounded-[2rem] p-6 shadow-sm">
          <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest mb-2">
            Total {isProvider ? "Earnings" : "Spent"}
          </p>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#059669]/10 text-[#059669] rounded-xl flex items-center justify-center">
              <Banknote size={20} />
            </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-20 text-center h-full flex-1">
            <div className="w-20 h-20 bg-brand-surface rounded-[2rem] flex items-center justify-center mb-6 border border-brand-outline">
              <History
                size={40}
                className="text-brand-text-variant opacity-50"
              />
            </div>
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead className="sticky top-0 bg-brand-surface-card z-10 border-b-2 border-brand-outline">
                <tr>
                    ...
                    <input
                      ...
                      onChange={() =>
                        setSelectedIds(
                          selectedIds.length === filteredHistory.length
                            ? []
                            : filteredHistory.map((j) => j.id),
                        )
                      }
                      className="w-5 h-5 rounded-lg border-2 border-brand-outline text-brand-primary cursor-pointer transition-all"
                    />
                  </th>
                  <th className="px-6 py-6 text-[10px] font-black text-brand-text-variant uppercase tracking-widest">
                    Date
                  </th>
                  <th className="px-6 py-6 text-[10px] font-black text-brand-text-variant uppercase tracking-widest">
                    Job & Party
                  ...
                  <th className="px-6 py-6 text-[10px] font-black text-brand-text-variant uppercase tracking-widest text-center">
                    Amount
                  </th>
                  <th className="px-6 py-6 text-[10px] font-black text-brand-text-variant uppercase tracking-widest text-center">
                    Payment
                  </th>
                  <th className="px-6 py-6 text-[10px] font-black text-brand-text-variant uppercase tracking-widest text-right">
                    Status
                  </th>
                </tr>
              </thead>
                ...
                {filteredHistory.map((job) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        {job.workerAvatar ? (
                          <div className="w-10 h-10 rounded-xl overflow-hidden border border-brand-outline shadow-sm shrink-0">
                            <img
                              src={
                                job.workerAvatar.startsWith("http")
                                  ? job.workerAvatar
                                  : `http://localhost:3000${job.workerAvatar}`
                              }
                              alt={job.worker}
                        ...
                        <div>
                          <h4 className="text-sm font-black text-brand-text-main group-hover:text-brand-primary transition-colors leading-tight mb-1">
                            {job.title}
                          </h4>
                    ...
                    <td className="px-6 py-6 text-center">
                      <span className="text-base font-black text-brand-text-main">
                    ...
                    <td className="px-6 py-6 text-right">
                      <div className="flex flex-col items-end gap-1.5">
                        <span
                          className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border shadow-sm ${
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-brand-surface-card w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-300">
            <div className="p-10">
              <div className="flex justify-between items-start mb-8">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[10px] font-bold text-brand-text-variant uppercase tracking-widest">
                      Job ID: #{selectedJob.id}
                    </span>
                  </div>
                  <h3 className="text-4xl font-black text-brand-text-main leading-tight mb-2 tracking-tight">
                    {selectedJob.title}
                  </h3>
                ...
                <div className="text-right shrink-0">
                  <p className="text-3xl font-black text-brand-text-main tracking-tight">
                  ...
                  <div>
                    ...
                    {selectedJob.rating > 0 ? (
                      <div className="bg-amber-50/50 p-6 rounded-[2rem] border border-amber-100 relative overflow-hidden h-full">
                        ...
                        <p className="text-sm text-amber-900 leading-relaxed italic font-bold">
                          "
                          {selectedJob.review ||
                      ...
                      <div className="bg-brand-surface p-6 rounded-[2rem] border border-brand-outline border-dashed flex flex-col items-center justify-center h-[140px] text-center">
                        <Star
                          size={24}
                          className="text-brand-text-variant opacity-30 mb-2"
                        />
                        <p className="text-[10px] font-bold text-brand-text-variant uppercase tracking-widest">
                          No Review Recorded
                        </p>
                      </div>
                    )}
                ...
                <div className="flex items-center gap-5">
                  <div className="relative">
                    {selectedJob.workerAvatar ? (
                      <img
                        src={
                          selectedJob.workerAvatar.startsWith("http")
                            ? selectedJob.workerAvatar
                            : `http://localhost:3000${selectedJob.workerAvatar}`
                        }
                  ...
                  <div>
                    <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest mb-1">
                      {selectedJob.role === "provider"
                        ? "Job Client"
                        : "Job Professional"}
                    </p>
                    <h5 className="text-xl font-black text-brand-text-main leading-none mb-2">
                      {selectedJob.worker || "Not Assigned"}
                    </h5>
                    <p className="text-sm font-bold text-brand-primary flex items-center gap-2">
                      <Phone size={14} /> {selectedJob.workerPhone || "No Phone"}
                    </p>
                  </div>
                </div>
                  ...
                  {selectedJob.status === "completed" && (
                    <button
                      onClick={() => {
                        ...
                        window.dispatchEvent(
                          new CustomEvent("change-tab", { detail: "jobs" }),
                        );
                        setSelectedJob(null);
                      }}
              ...
              <button
                onClick={() => setSelectedJob(null)}
                className="w-full mt-10 py-4 text-[10px] font-black uppercase text-brand-text-variant tracking-[0.2em] hover:text-brand-text-main transition-colors"
              >
