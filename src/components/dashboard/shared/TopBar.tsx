...
export const TopBar = ({ isDark, toggleTheme, role = 'client' }: { isDark: boolean, toggleTheme: () => void, role?: 'client' | 'provider' }) => {
  ...
  const displayAvatar = userProfile?.avatar_url 
    ? (userProfile.avatar_url.startsWith('http') ? userProfile.avatar_url : `http://localhost:3000${userProfile.avatar_url}`)
    : null;
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-3 w-80 bg-brand-surface border border-brand-outline rounded-2xl shadow-xl z-50 overflow-hidden">
                  <div className="p-4 border-b border-brand-outline flex justify-between items-center bg-brand-surface-container/50">
                    <h3 className="text-sm font-bold text-brand-text-main">Notifications</h3>
                    <button 
                      onClick={() => {
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        ...
                        >
                          {!n.is_read && <div className="absolute left-1 top-1/2 -translate-y-1/2 w-1 h-8 bg-brand-primary rounded-full"></div>}
                          <p className="text-sm font-semibold text-brand-text-main mb-0.5">{n.title}</p>
                          <p className="text-xs text-brand-text-variant leading-relaxed line-clamp-2">{n.message}</p>
