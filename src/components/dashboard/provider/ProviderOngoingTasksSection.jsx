...
export const ProviderOngoingTasksSection = () => {
  const [activeSupportJob, setActiveSupportJob] = React.useState(null);
  ...
  const fetchTasks = React.useCallback(async () => {
    try {
      ...
      const appliedJobIds = Array.isArray(myApplications)
        ? myApplications.map((a) => a.job_id)
        : [];
  ...
  return (
    <div className="max-w-[1400px] mx-auto py-12 px-12 w-full">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        <div>
          <h1 className="text-3xl font-bold text-brand-text-main mb-2 tracking-tight">
            Operation Command
          </h1>
          <p className="text-sm text-brand-text-variant font-medium">
            Manage active jobs, follow safety protocols, and track progress.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-12">
        {tasks.map((task) => (
          <div
            key={task.id}
            className="bg-brand-surface-card border border-brand-outline rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col lg:flex-row min-h-[400px]"
          >
            {/* Left Info Pane */}
            <div className="flex-1 p-10 flex flex-col border-r border-brand-outline bg-brand-surface/20">
              ...
              <h3 className="text-3xl font-bold text-brand-text-main mb-6 leading-tight">
                {task.title}
              </h3>
              ...
              <div className="space-y-4 mb-10">
                ...
                >
                  <User
                    size={18}
                    className="text-brand-primary group-hover/client:scale-110 transition-transform"
                  />
                  <span>
                    Client:{" "}
                    <span className="font-bold border-b border-dashed border-brand-outline group-hover/client:border-brand-primary">
                      {task.client_name}
                    </span>
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3 text-sm font-bold text-orange-600">
                  <Clock size={18} />
                  <span>
                    Started {new Date(task.created_at).toLocaleDateString()}
                  </span>
                </div>
              </div>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        await api.sendMessage(
                          task.client_id,
                          "I have completed the work. Please review and release the funds.",
                          task.id,
                        );
                        alert(
                          "Work submitted for review! The client has been notified to release your payment.",
                        );
                        setTasks((prev) =>
                          prev.map((t) =>
                            t.id === task.id ? { ...t, is_submitted: true } : t,
                          ),
                        );
                      } catch (err) {
              ...
              <div className="mt-auto space-y-3">
                <div className="p-4 bg-orange-50 border border-orange-100 rounded-2xl mb-4">
                  ...
                  <p className="text-[11px] text-orange-800 leading-relaxed italic">
                    For your safety, always keep communication and payment
                    records within Lingkod Hub. External agreements waive our
                    dispute protection.
                  </p>
                </div>
                ...
                <button className="w-full py-3 text-left px-5 bg-brand-surface border border-brand-outline rounded-2xl text-[11px] font-bold text-brand-text-main hover:border-brand-primary transition-all flex items-center justify-between group">
                  ...
                  <ChevronRight
                    size={14}
                    className="text-brand-text-variant group-hover:translate-x-1 transition-transform"
                  />
                </button>
                <button className="w-full py-3 text-left px-5 bg-brand-surface border border-brand-outline rounded-2xl text-[11px] font-bold text-brand-text-main hover:border-brand-primary transition-all flex items-center justify-between group">
