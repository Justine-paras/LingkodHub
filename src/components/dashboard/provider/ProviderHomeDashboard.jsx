...
export const ProviderHomeDashboard = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [ongoing, assigned, history, available, reviews] =
          await Promise.all([
        ...
        const invitations = assignedList.filter(
          (j) =>
        ...
        const uniqueInvitations = Array.from(
          new Map(invitations.map((j) => [j.id, j])).values(),
        );
        ...
        const validAssigned = assignedList.filter((j) => {
          if (j.status === "in_progress") return true;
          if (j.status === "pending" && appliedJobIds.includes(j.id))
            return true;
          return false;
        });
  ...
  const lifetimeEarnings = completedJobs.reduce(
  ...
  return (
    <div className="flex flex-col h-full px-12 py-8 gap-10 max-w-[1400px] mx-auto w-full relative">
        ...
        <div className="absolute inset-0 z-50 bg-brand-surface/60 backdrop-blur-sm flex items-center justify-center p-8 rounded-3xl">
          <div className="bg-brand-surface-card border-2 border-brand-outline rounded-[2.5rem] p-10 max-w-lg text-center shadow-2xl">
            ...
            <p className="text-brand-text-variant mb-8 leading-relaxed">
              To start accepting jobs and offering your services, please verify
              your email address. This helps us ensure a safe marketplace for
              everyone.
            </p>
            <button
              onClick={() =>
      ...
      <header
        className={`flex flex-col md:flex-row md:items-end justify-between gap-8 pb-4 ${!isEmailVerified ? "opacity-50 pointer-events-none select-none" : ""}`}
      >
        <div>
          <h1 className="text-4xl font-extrabold text-brand-text-main tracking-tight mb-2">
            Hello, {firstName}!
          </h1>
          <p className="text-brand-text-variant font-semibold flex items-center gap-2">
            You have{" "}
            <span className="text-brand-primary font-bold">
              {ongoingJobs.length} active jobs
            </span>{" "}
            currently.
            <span className="inline-flex items-center gap-1.5 ml-2">
              <span
                className={`w-2 h-2 rounded-full ${isOnline ? "bg-[#059669] animate-pulse" : "bg-gray-400"}`}
              ></span>
        ...
        <div className="flex items-center gap-4 bg-brand-surface-card border-2 border-brand-outline p-2 rounded-[2rem] shadow-sm hover:shadow-md transition-shadow">
          <div className="px-8 py-3 border-r border-brand-outline">
            <p className="text-[10px] font-bold text-brand-text-variant uppercase tracking-widest mb-1 opacity-70">
              Available Balance
            </p>
      ...
      {/* Main Dashboard Grid */}
      <div
        className={`grid grid-cols-1 lg:grid-cols-12 gap-8 w-full ${!isEmailVerified ? "opacity-50 pointer-events-none select-none" : ""}`}
      >
            ...
            <section className="animate-in slide-in-from-top-4 duration-500">
              ...
              <div className="space-y-4">
                {pendingInvitations.map((inv) => (
                  <div
                    key={inv.id}
                    className="bg-white border-2 border-amber-200 rounded-3xl p-6 shadow-xl shadow-amber-500/5 flex flex-col md:flex-row md:items-center justify-between gap-6"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-lg font-black text-brand-text-main leading-tight mb-1">
                          {inv.title}
                        </h4>
                        <p className="text-xs font-bold text-brand-text-variant uppercase">
                          <span
                            onClick={() => setSelectedClientId(inv.client_id)}
                            className="cursor-pointer border-b border-dashed border-brand-outline hover:border-brand-primary hover:text-brand-primary transition-colors text-brand-text-main"
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            window.dispatchEvent(
                              new CustomEvent("dashboard_sync", {
                                detail: { type: "DATA_UPDATED" },
                              }),
                            );
          ...
          <section>
            ...
            {nextUpJob ? (
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                      <h3 className="text-2xl font-bold text-brand-text-main mb-2">
                        {nextUpJob.title}
                      </h3>
                      <div className="flex items-center gap-4 text-sm text-brand-text-variant flex-wrap">
                        <span
                          onClick={() =>
                            setSelectedClientId(nextUpJob.client_id)
                          }
                        ...
                        <span className="flex items-center gap-1.5">
                          <MapPin size={16} /> {nextUpJob.location}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    <button
                      onClick={() =>
                        window.open(
                          `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(nextUpJob.location)}`,
                        )
                      }
                        ...
                        onClick={async () => {
                          if (!nextUpJob.provider_arrived) {
                            try {
                              ...
                              channel.close();
                              alert(
                                nextUpJob.client_confirmed
                                  ? "Arrival confirmed! Task is now in progress."
                                  : "You marked your arrival. Waiting for client to confirm.",
                              );
              ...
              <div className="bg-brand-surface border-2 border-dashed border-brand-outline rounded-[2.5rem] p-12 text-center">
                <p className="text-brand-text-variant font-medium mb-4">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {availableJobs.slice(0, 4).map((job, i) => (
                <div
                  key={job.id}
                  className="bg-brand-surface-card border border-brand-outline p-6 rounded-3xl hover:border-brand-primary transition-all group cursor-pointer shadow-sm"
                >
                  <div className="flex justify-between items-start mb-4">
                    <span className="px-2 py-0.5 bg-brand-surface border border-brand-outline text-brand-text-variant text-[10px] font-bold rounded-md uppercase tracking-wider">
                      Open Job
                    </span>
                  ...
                  <h3 className="text-lg font-bold text-brand-text-main mb-2 leading-tight group-hover:text-brand-primary transition-colors line-clamp-1">
                    {job.title}
                  </h3>
              ...
              {availableJobs.length === 0 && (
                <p className="text-sm text-brand-text-variant col-span-2">
                  No new jobs matching your profile right now.
                </p>
              )}
            </div>
          </section>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          ...
          <div className="bg-brand-surface-card border border-brand-outline rounded-[2.5rem] p-8 shadow-sm flex flex-col">
            <h3 className="text-sm font-bold text-brand-text-main uppercase tracking-widest mb-8 border-b border-brand-outline pb-4">
            ...
            <div className="space-y-8">
              <div>
                <div className="flex justify-between items-end mb-2">
                  <p className="text-xs font-semibold text-brand-text-variant uppercase tracking-wider">
                    Reputation Partner
                  </p>
                  <p className="text-2xl font-bold text-brand-text-main">
                    ...
                    <Star
                      size={16}
                      className="inline fill-amber-400 text-amber-400 relative top-[-2px] ml-0.5"
                    />
                  </p>
                </div>
                <div className="h-1.5 w-full bg-brand-surface-container rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${(stats.avg_rating / 5) * 100}%` }}
                    transition={{ duration: 1 }}
                    className="h-full bg-amber-400 rounded-full"
                  />
                </div>
                <p className="text-[10px] text-brand-text-variant mt-2 font-medium">
              ...
              <div>
                ...
                <p className="text-[10px] text-brand-text-variant mb-6 font-medium">
                  Auto-payout scheduled for Friday
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-brand-surface border border-brand-outline rounded-2xl text-center">
                    <p className="text-[10px] uppercase font-bold text-brand-text-variant tracking-tighter mb-0.5">
                      Jobs Done
                    </p>
                    <p className="text-lg font-bold text-brand-text-main">
                      {completedCount}
                    </p>
                  </div>
                  <div className="p-3 bg-brand-surface border border-brand-outline rounded-2xl text-center">
...
export const BrowseJobsSection = () => {
  ...
  React.useEffect(() => {
    Promise.all([api.getJobs(), api.getServices()])
      .then(([jobsData, servicesData]) => {
        ...
        setCategories(["All", ...servicesData.map((s) => s.name)]);
      })
      .catch(console.error)
      .finally(() => setIsLoading(false));
  }, []);

  const sendOffer = async (jobId, e) => {
    const btn = e.currentTarget;
    try {
      btn.disabled = true;
      btn.innerText = "Sending...";
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-0 w-full">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        <div>
          ...
          <p className="text-sm text-brand-text-variant">
            Find work that matches your skills in your neighborhood
          </p>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {isLoading ? (
          <p className="text-brand-text-variant text-sm">
          ...
          filteredJobs.map((job) => (
            ...
            >
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <span className="px-3 py-1 bg-brand-primary/10 text-brand-primary text-[10px] font-bold rounded-lg uppercase tracking-wider">
                    Open Job
                  </span>
                  <span className="text-[10px] text-brand-text-variant font-medium flex items-center gap-1.5">
                    <Clock size={12} />{" "}
                    {new Date(job.created_at).toLocaleDateString()}
                  </span>
                </div>
                <h3 className="text-2xl font-bold text-brand-text-main mb-3 leading-tight group-hover:text-brand-primary transition-colors">
                  {job.title}
                </h3>
              ...
              <p className="text-sm text-brand-text-variant leading-relaxed line-clamp-2 mb-8">
                {job.description}
              </p>
              ...
              <div className="mt-auto flex items-center justify-between pt-6 border-t border-brand-outline">
                ...
                <button
                  onClick={(e) => sendOffer(job.id, e)}
                  className="px-8 py-3 bg-brand-primary text-white text-sm font-bold rounded-2xl hover:bg-[#059669] transition-all shadow-lg hover:shadow-brand-primary/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Send Offer
                </button>
              </div>
