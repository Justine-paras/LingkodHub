...
export const ProviderInvitationsSection = () => {
  ...
  const fetchInvitations = async () => {
    try {
      ...
      const uniqueInvites = Array.from(new Map(combined.map(j => [j.id, j])).values());
      ...
      const filtered = uniqueInvites.filter((j: any) => 
  ...
  const handleDecision = async (jobId: number, status: 'accepted' | 'rejected') => {
    try {
      if (status === 'accepted') {
        // 1. Send application/acceptance message
        await api.applyToJob(jobId, 'I accept your direct work invitation! Let\'s get started.');
        ...
        alert('Invitation accepted! Please check your Ongoing Tasks to confirm arrival.');
      } else {
    ...
    <div className="max-w-6xl mx-auto py-12 px-6 lg:px-12 w-full flex flex-col min-h-screen font-sans">
      <div className="mb-12 text-left">
         <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-brand-text-main tracking-tight uppercase">Work Invitations</h1>
         </div>
         <p className="text-brand-text-variant font-medium">Direct requests from clients who specifically want your expertise.</p>
      </div>
      ...
      {isLoading ? (
        <div className="py-32 flex flex-col items-center justify-center text-center">
           ...
           <p className="text-brand-text-variant font-bold animate-pulse uppercase tracking-widest text-sm">Reviewing your inbox...</p>
        </div>
        ...
        <div className="py-12 flex flex-col items-center justify-center">
           <div className="w-full max-w-2xl bg-brand-surface-card border-2 border-brand-outline rounded-[3rem] p-12 text-center shadow-xl">
              ...
              <h2 className="text-3xl font-black text-brand-text-main mb-4">Inbox is quiet 📥</h2>
              <p className="text-brand-text-variant text-lg leading-relaxed">
                 You don't have any direct work invitations yet. Keep your profile updated and your status "Online" to attract more direct clients.
              </p>
           </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 content-start pb-20">
          {invitations.map(inv => (
            <div key={inv.id} className="group bg-brand-surface-card border-2 border-brand-outline hover:border-brand-primary p-8 rounded-[2.5rem] transition-all relative flex flex-col shadow-sm hover:shadow-2xl hover:shadow-brand-primary/5">
               {/* Badge */}
               <div className="flex justify-between items-start mb-6">
                  ...
                  <span className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest flex items-center gap-1.5">
                     <Clock size={12} /> {new Date(inv.created_at).toLocaleDateString()}
                  </span>
               </div>
               ...
               <div className="mb-8 flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="flex items-center gap-3 p-4 bg-brand-surface border border-brand-outline rounded-2xl shadow-sm">
                        ...
                        <div>
                           <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest">Location</p>
                           <p className="text-xs font-bold text-brand-text-main truncate">{inv.location}</p>
                        </div>
                     </div>
                     <div className="flex items-center gap-3 p-4 bg-brand-surface border border-brand-outline rounded-2xl shadow-sm">
                        <Calendar size={18} className="text-brand-primary shrink-0" />
                        <div>
                           <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest">Schedule</p>
                           <p className="text-xs font-bold text-brand-text-main truncate">{inv.scheduled_at ? new Date(inv.scheduled_at).toLocaleDateString() : 'ASAP'}</p>
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div className="flex items-center justify-between pt-6 border-t border-brand-outline/50 mb-8">
                  ...
                  <div className="text-right">
                     <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest mb-1">Offered Budget</p>
                     <p className="text-2xl font-black text-[#059669]">₱{inv.budget?.toLocaleString()}</p>
                  </div>
               </div>
