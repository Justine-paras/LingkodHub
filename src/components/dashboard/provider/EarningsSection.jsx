...
import { TrendingUp, Smartphone, Star } from "lucide-react";
...
export const EarningsSection = () => {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      ...
      const enriched = historyList.map((job) => {
        const review = Array.isArray(reviews)
          ? reviews.find((r) => r.job_id === job.id)
          : null;
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-12 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <div className="lg:col-span-2 bg-brand-surface-card border border-brand-outline rounded-[2rem] p-8 shadow-sm flex flex-col justify-center min-h-[300px]">
          <div className="flex flex-col items-center">
            ...
            <p className="text-sm text-brand-text-variant font-medium mb-8">
              Your revenue is calculated based on completed tasks.
            </p>
            ...
            <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                },
                {
                  label: "Avg Rate",
                  value: completedJobs.length
                    ? (totalEarned / completedJobs.length).toFixed(0)
                    : 0,
                  unit: "₱",
                },
                ...
                <div
                  key={i}
                  className="p-4 bg-brand-surface border border-brand-outline rounded-2xl"
                >
                  ...
                  <p className="text-[8px] text-brand-text-variant uppercase font-bold mt-1 opacity-60">
                    {stat.unit}
                  </p>
                </div>
              ))}
        ...
        <div className="bg-brand-surface-card border border-brand-outline rounded-[2rem] p-8 shadow-sm">
          <h3 className="text-lg font-bold text-brand-text-main mb-6">
            Wallet Summary
          </h3>
          <div className="space-y-6">
            ...
            <div className="pt-6 border-t border-brand-outline mt-6">
              <label className="text-[10px] font-bold text-brand-text-variant uppercase tracking-widest block mb-4">
                Saved Payout Method
              </label>
              <div className="flex items-center gap-3 p-3 bg-brand-surface border border-brand-outline rounded-xl">
                ...
                <div>
                  <p className="text-xs font-bold text-brand-text-main capitalize">
                    {paymentMethod === "none"
      ...
      <div className="bg-brand-surface-card border border-brand-outline rounded-[2rem] overflow-hidden shadow-sm">
        <div className="p-6 border-b border-brand-outline bg-brand-surface/50 font-bold text-sm text-brand-text-main">
          Transaction History
        </div>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-brand-outline">
                <th className="px-6 py-4 font-bold text-brand-text-variant uppercase text-[10px] tracking-widest">
                  Date
                </th>
                <th className="px-6 py-4 font-bold text-brand-text-variant uppercase text-[10px] tracking-widest">
                  Job
            ...
            <tbody className="divide-y divide-brand-outline/50">
              {completedJobs.length === 0 ? (
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-12 text-center text-brand-text-variant italic font-medium"
                  >
                ...
                completedJobs.map((tx) => (
                  ...
                  >
                    <td className="px-6 py-4 font-medium text-brand-text-variant whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 font-extrabold text-[#059669]">
                      +₱
                      {Number(tx.budget || 0).toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}
                    </td>
                    <td className="px-6 py-4">
