...
export const ProviderProfileSettings = () => {
  ...
  const [identityIdFile, setIdentityIdFile] = React.useState(null);
  const [identityIdUrl, setIdentityIdUrl] = React.useState("");
  const [identitySelfieFile, setIdentitySelfieFile] = React.useState(null);
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setStats(reviews);
        setServices(userServices.map((s) => s.name));
      } catch (error) {
  ...
  const handleIdentitySubmit = async () => {
    ...
    try {
      const { status, is_documents_verified } = await api.uploadDocuments(
        identityIdFile,
        identitySelfieFile,
      );
      setDocumentStatus(status);
  ...
  return (
    <div className="max-w-5xl mx-auto py-16 px-12">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-sm text-brand-text-variant font-medium">
                    Update the services you can offer to clients
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="relative mb-6">
                <Search
                  size={18}
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-text-variant"
                />
                <input
                  type="text"
      ...
      <header className="flex justify-between items-end mb-12">
        <div>
          <h1 className="text-3xl font-semibold text-brand-text-main mb-2">
            Account Settings
          </h1>
          <p className="text-sm text-brand-text-variant">
            Manage your professional account and personal details
          </p>
        </div>
        <div className="flex flex-col items-end gap-2 text-xs font-medium text-brand-text-variant mt-4 sm:mt-0">
          ...
          <div className="flex items-center gap-2">
            {isEmailVerified ? (
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-brand-text-variant italic">
                  Email unverified
                </span>
                {!isOTPSent ? (
                  <button
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-brand-surface-card border border-brand-outline p-8 rounded-2xl text-center shadow-sm">
            ...
            <div className="flex items-center justify-center gap-2 mb-1 group max-w-[200px] mx-auto min-h-[32px]">
              {isEditingName ? (
                ...
                <>
                  <h4
                    className="text-lg font-medium text-brand-text-main truncate"
                    title={displayName}
                  >
                    {displayName}
                  </h4>
                  <button
                    ...
                    className="text-brand-text-variant opacity-0 group-hover:opacity-100 hover:text-brand-primary transition-all p-1 -mr-6 shrink-0"
                    title="Edit Name"
                  >
                    <Edit2 size={16} />
                  </button>
          ...
          <div className="bg-brand-surface-card border border-brand-outline p-6 rounded-2xl shadow-sm">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-sm font-semibold text-brand-text-main">
                Profile Completion
              </h3>
            ...
            {completion.missing.length > 0 && (
              <div>
                <p className="text-[10px] font-bold text-brand-text-variant mb-2 uppercase tracking-wider">
                  Missing Items
                </p>
                <ul className="space-y-2">
                  {completion.missing.map((item) => (
                    <li
                      key={item}
                      className="flex items-center gap-2 text-xs text-brand-text-main group cursor-pointer hover:text-brand-primary transition-colors"
                    >
          ...
          <div className="bg-brand-surface-card border border-brand-outline p-6 rounded-2xl shadow-sm">
            <h3 className="text-sm font-semibold text-brand-text-main mb-4">
              Provider Stats
            </h3>
        ...
        <div className="md:col-span-2 bg-brand-surface-card border border-brand-outline rounded-2xl shadow-sm flex flex-col overflow-hidden">
          <div className="p-8 space-y-8 flex-1">
            {activeSettingsTab === "profile" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <InputGroup
                  label="Email Address"
                  helper="You cannot change your email address right now."
                >
                  <input
                    type="email"
                    value={email}
                ...
                <InputGroup label="Location">
                  <div className="relative">
                    <MapPin
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-text-variant"
                      size={16}
                    />
                    <input
                      type="text"
                ...
                <div className="col-span-1 md:col-span-2 space-y-4 pt-4 border-t border-brand-outline">
                  <div className="flex justify-between items-center">
                    <h3 className="text-sm font-bold text-brand-text-main uppercase tracking-widest">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {services.map((service) => (
                      <div
                        key={service}
                        className="flex items-center gap-2 bg-brand-primary/10 border border-brand-primary/20 px-4 py-2 rounded-xl transition-all hover:bg-brand-primary/15"
                      >
                    ...
                    {services.length === 0 && (
                      <p className="text-xs text-brand-text-variant font-medium italic">
                        No services selected. Add services to start receiving
                        job alerts.
                      </p>
                    )}
                  </div>
            ...
            {activeSettingsTab === "security" && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-bold text-brand-text-main uppercase tracking-widest mb-4">
                    Security
                  </h3>
                  <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-brand-surface border-2 border-brand-outline rounded-2xl p-6 shadow-sm">
                    <div>
                      <h4 className="text-sm font-semibold text-brand-text-main mb-1">
                        Account Password
                      </h4>
                  ...
                  <div className="bg-brand-surface border-2 border-brand-outline rounded-2xl flex flex-col shadow-sm">
                    <div className="flex items-center justify-between p-6 border-b border-brand-outline">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Receive an email when clients message you.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setPrefEmailMessages(e.target.checked ? 1 : 0)
                          }
                        />
                    ...
                    <div className="flex items-center justify-between p-6 border-b border-brand-outline">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Get notified when a job status changes or an offer is
                          made.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setPrefEmailUpdates(e.target.checked ? 1 : 0)
                          }
                        />
                    ...
                    <div className="flex items-center justify-between p-6">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Receive emails about new features, discounts, and
                          platform updates.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setPrefEmailPromos(e.target.checked ? 1 : 0)
                          }
                        />
                  ...
                  <div className="bg-brand-surface border-2 border-brand-outline rounded-2xl flex flex-col shadow-sm">
                    <div className="flex items-center justify-between p-6 border-b border-brand-outline">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Allow the website to send you browser push
                      ...
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setPrefPushAlerts(e.target.checked ? 1 : 0)
                          }
                        />
                    ...
                    <div className="flex items-center justify-between p-6">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Receive occasional push notifications about local
                      ...
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setPrefPushMarketing(e.target.checked ? 1 : 0)
                          }
                        />
                  ...
                  <div className="bg-brand-surface border-2 border-brand-outline rounded-2xl flex flex-col shadow-sm">
                    <div className="flex items-center justify-between p-6 border-b border-brand-outline">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Allow clients to find and view your public provider
                          profile.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setIsPublicProfile(e.target.checked ? 1 : 0)
                          }
                        />
                    ...
                    <div className="flex items-center justify-between p-6">
                      <div className="pr-4">
                        ...
                        <p className="text-xs text-brand-text-variant">
                          Show others when you are currently active on the
                          platform.
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer outline-none">
                        <input
                          ...
                          onChange={(e) =>
                            setShowOnlineStatus(e.target.checked ? 1 : 0)
                          }
                        />
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-red-600/80">
                          Permanently remove your provider account and all
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* GCash */}
                    <div
                      className={`p-6 rounded-2xl border-2 transition-all ${preferredPayment === "gcash" ? "border-brand-primary bg-brand-primary/5" : "border-brand-outline bg-brand-surface"}`}
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="w-12 h-8 bg-blue-600 rounded flex items-center justify-center font-bold text-white text-[10px] tracking-tighter">
                          GCash
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <p className="text-[10px] font-bold text-brand-text-variant uppercase mb-1">
                            GCash Number
                          </p>
                          <input
                            type="text"
                    ...
                    {/* Maya */}
                    <div
                      className={`p-6 rounded-2xl border-2 transition-all ${preferredPayment === "maya" ? "border-brand-primary bg-brand-primary/5" : "border-brand-outline bg-brand-surface"}`}
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="w-12 h-8 bg-green-500 rounded flex items-center justify-center font-bold text-white text-[10px] tracking-tighter">
                          MAYA
                        </div>
                ...
                <div>
                  ...
                  <div className="bg-brand-surface border-2 border-brand-outline rounded-2xl p-6 shadow-sm">
                    <p className="text-xs text-brand-text-variant mb-6">
                      Set the maximum distance you are willing to travel for
                      service requests from your base.
                    </p>
                    <input
                      type="range"
                      min="1"
          ...
          <div className="bg-brand-surface-container/30 px-8 py-6 flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-brand-outline">
            <div className="flex-1 text-xs font-medium text-brand-text-variant text-center sm:text-left">
              <AnimatePresence mode="wait">
                {showSuccess ? (
                  ...
                  <p>
                    Private data will never be shared without your permission.
                  </p>
                )}
              </AnimatePresence>
            </div>
            <div className="flex gap-3 items-center shrink-0">
              <button
                type="button"
                className="px-6 py-2.5 text-sm font-semibold text-brand-text-variant hover:text-brand-text-main hover:bg-brand-text-variant/10 rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
          ...
          <button
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent("change-tab", { detail: "help" }),
              )
            }
            className="hover:text-brand-text-main transition-colors outline-none"
          >
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-sm text-brand-text-variant mb-6">
                This will permanently remove your profile, earnings history, and
                active applications. To confirm, please type{" "}
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-brand-text-main flex items-center gap-2">
                    <Shield size={24} className="text-brand-primary" /> Verify
                    Your Identity
                  </h2>
                  <p className="text-sm text-brand-text-variant font-medium">
                    Upload ID and a selfie to get Verified instantly.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="overflow-y-auto pr-2 custom-scrollbar space-y-6 flex-1">
                <div className="p-4 bg-brand-primary/5 border border-brand-primary/20 rounded-2xl">
                  <p className="text-xs text-brand-text-variant font-medium leading-relaxed">
                    To safeguard our marketplace, we require a valid government
                    ID and matching verification selfie. Submitted documents are
                    processed instantly.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div
                    onClick={() => {
                      if (!identityIdUrl) {
                        const input =
                          document.getElementById("settings-id-input");
                        input?.click();
                      }
                    }}
                  ...
                  >
                    ...
                    {identityIdUrl ? (
                      ...
                      <>
                        ...
                        <span className="text-[10px] font-bold text-brand-text-variant uppercase tracking-widest group-hover:text-brand-primary transition-colors">
                          ID Front View
                        </span>
                      </>
                    )}
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-brand-text-main ml-1 uppercase tracking-widest">
                    Document Type
                  </label>
                  <select
              ...
              <div className="mt-6 pt-6 border-t border-brand-outline flex gap-4">
                ...
                <button
                  ...
                  disabled={
                    isSubmittingIdentity || !identityIdUrl || !identitySelfieUrl
                  }
...
const BillingHistory = () => {
  ...
  if (isLoading)
    return (
      <div className="text-xs text-brand-text-variant italic">
        Loading history...
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="bg-brand-surface border-2 border-brand-outline rounded-2xl flex flex-col shadow-sm overflow-hidden">
        {payments.length === 0 ? (
          <div className="p-12 text-center text-brand-text-variant text-sm">
            No payouts yet.
          </div>
        ) : (
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-brand-outline text-brand-text-main">
              {payments.map((p) => (
                <tr
                  key={p.id}
                  className="hover:bg-brand-surface-card transition-colors"
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${p.status === "completed" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                    >
                      {p.status}
                    </span>
                  </td>
