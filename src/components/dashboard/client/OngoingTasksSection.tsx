...
export const OngoingTasksSection = () => {
  ...
  const fetchTasks = React.useCallback(async () => {
    try {
      ...
      const assignedPendingCandidates = pendingJobs.filter((j: any) => j.provider_id !== null);
      ...
      const validAssignedPending = assignedPendingResults.filter(j => j !== null);
      ...
      const enrichedTasks = await Promise.all(unique.map(async (job) => {
        try {
  ...
  const completeTask = async (job: any) => {
    ...
    try {
      ...
      if (job.status === 'pending') {
        try {
          ...
          const providerApp = applications.find((a: any) => Math.floor(a.provider_id) === Math.floor(job.provider_id));
          if (providerApp && providerApp.status !== 'accepted') {
            await api.decideApplication(Math.floor(providerApp.id), 'accepted', job.payment_method);
          }
      ...
      setRating(5);
      setReviewComment('');
      alert('Funds released and review submitted! Thank you for using LingkodHub.');
    } catch (error) {
      console.error('Failed to complete task', error);
      alert(`Payment Release Failed: The system encountered an error. This can happen if the professional has not officially accepted the job on their dashboard yet.`);
    } finally {
    ...
    <div className="max-w-4xl mx-auto py-12 px-12 w-full flex flex-col min-h-[calc(100vh-80px)]">
        ...
        {isLoading ? (
           <div className="flex flex-col items-center justify-center bg-brand-surface-card border border-brand-outline rounded-3xl p-16 text-center shadow-sm h-full max-h-[400px]">
            ...
            activeTasks.map((task) => (
               <div key={task.id} className="bg-brand-surface-card border border-brand-outline rounded-[2rem] shadow-lg overflow-hidden flex flex-col group relative">
                  ...
                  {task.is_submitted && (
                    <div className="bg-amber-500 text-white px-6 py-2.5 flex items-center justify-between shadow-inner">
                       ...
                       <span className="text-[10px] font-bold bg-white/20 px-2 py-0.5 rounded">Action Required</span>
                    </div>
                  )}
                  ...
                  <div className="p-8 flex flex-col md:flex-row md:items-start gap-8 relative">
                     ...
                     <div className="md:absolute md:top-8 md:right-8 flex md:flex-col items-center md:items-end justify-between md:justify-start gap-2 w-full md:w-auto mb-4 md:mb-0">
                        <div className="flex flex-col md:items-end">
                           <span className="text-2xl font-black text-[#059669]">₱{Number(task.budget || 0).toLocaleString()}</span>
                     ...
                     <div className="flex-1 text-center md:text-left pt-1">
                        <h3 className="text-3xl font-bold text-brand-text-main mb-4 leading-tight pr-0 md:pr-40">{task.title}</h3>
                  ...
                  {task.status === 'pending' ? (
                     <button 
                         onClick={async () => {
                            if (!task.client_confirmed) {
                               try {
                                 await api.sendMessage(task.provider_id, '[SYSTEM:CLIENT_CONFIRMED_ARRIVAL]', task.id);
                                 ...
                                 channel.postMessage({ type: 'DATA_UPDATED' });
                                 channel.close();
                                 alert(task.provider_arrived ? 'Worker confirmed! Task is now in progress.' : 'You confirmed arrival. Waiting for worker to also confirm.');
                                 fetchTasks();
                               } catch(e) {
                                 console.error(e);
                               }
                     ...
                     >
                        <MapPin size={20} />
                        {task.client_confirmed ? 'Waiting for Professional to arrive...' : 'Confirm Worker is Here'}
                     </button>
                  ) : (
                     <button 
                     ...
                     >
                        {task.is_submitted ? <CheckCircle size={20} /> : <MessageSquare size={18} />}
                        {task.is_submitted ? 'Release Funds & Rate Professional' : 'Review & Release Funds'}
                     </button>
                  )}
               </div>
            ))
      ...
      {showCompletionModal && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-md animate-in fade-in duration-300" onClick={() => !isSubmitting && setShowCompletionModal(null)}></div>
            <div className="relative bg-brand-surface-card w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-bottom-8 duration-300">
               <div className="p-10">
                  ...
                  <h3 className="text-3xl font-black text-brand-text-main mb-2 text-center">Rate & Release Funds</h3>
                  <p className="text-brand-text-variant mb-10 text-center leading-relaxed font-medium">
                     How was your experience with <span className="text-brand-text-main font-bold">{showCompletionModal.provider_name || 'the professional'}</span>?
                  </p>
                  ...
                  {/* Review Box */}
                  <div className="mb-10">
                    <label className="block text-[10px] font-black text-brand-text-variant uppercase tracking-widest mb-3 ml-2">Your Feedback</label>
                    <textarea 
                      value={reviewComment}
                  ...
                  <div className="flex flex-col gap-4">
                     ...
                     >
                        {isSubmitting ? 'Processing Payment...' : (
                          <>
                            <Banknote size={20} />
                            Pay ₱{Number(showCompletionModal.budget || 0).toLocaleString()} & Release
                          </>
                        )}
                     </button>
                     <button 
      ...
      {showCancelModal && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-brand-surface-card w-full max-w-md rounded-[2.5rem] p-10 text-center shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <h3 className="text-2xl font-bold text-brand-text-main mb-4">Cancel this task?</h3>
                <p className="text-brand-text-variant mb-8 text-sm">This will remove the job from your active list and notify the professional.</p>
                <div className="flex gap-4">
                    <button onClick={() => setShowCancelModal(null)} className="flex-1 py-4 bg-brand-surface border border-brand-outline rounded-xl font-bold text-xs uppercase">Stay</button>
                    <button onClick={() => cancelTask(showCancelModal.id)} className="flex-1 py-4 bg-red-600 text-white rounded-xl font-bold text-xs uppercase">Cancel Job</button>
                </div>
            </div>
         </div>
      )}
      ...
      {showReportModal && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-brand-surface-card w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200">
                <h3 className="text-xl font-bold text-brand-text-main mb-6">Report Issue</h3>
                <textarea className="w-full bg-brand-surface border border-brand-outline rounded-2xl p-4 mb-6 h-32" placeholder="What happened?" />
                <div className="flex gap-4">
                    <button onClick={() => setShowReportModal(null)} className="flex-1 py-3 bg-brand-surface border border-brand-outline rounded-xl font-bold text-xs">Back</button>
                    <button onClick={() => setShowReportModal(null)} className="flex-1 py-3 bg-orange-500 text-white rounded-xl font-bold text-xs">Submit</button>
                </div>
            </div>
         </div>
      )}
