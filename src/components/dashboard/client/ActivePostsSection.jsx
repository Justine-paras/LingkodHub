...
export const ActivePostsSection = ({
  ...
  const [viewApplicantsJob, setViewApplicantsJob] = React.useState(null);
  const [showConfetti, setShowConfetti] = React.useState(false);
  const [showPostModal, setShowPostModal] = React.useState(false);
  const [showSuccessToast, setShowSuccessToast] = React.useState(false);
  const [showDeleteModal, setShowDeleteModal] = React.useState(null);
  const [isLoading, setIsLoading] = React.useState(true);
  const [activePosts, setActivePosts] = React.useState([]);
  ...
  const [invitedProvider, setInvitedProvider] = React.useState(null);
  ...
  React.useEffect(() => {
    if (initialInvitedProvider) {
      ...
      if (initialInvitedProvider.services) {
        ...
        setFormTitle(
          `${initialInvitedProvider.services.split(", ")[0] || "Task"} Invitation`,
        );
      }
      onClearPending();
    }
  ...
  const fetchJobs = React.useCallback(async () => {
    try {
      ...
      const activePostsFiltered = jobsWithApps.filter((job) => {
        ...
        const providerApplied = job.applicants.some(
  ...
  const handlePostJob = async () => {
    ...
    const currentPaymentNum =
      formPayment === "gcash" ? tempGcashNumber : tempMayaNumber;
    if (!currentPaymentNum) {
      setFormError(
        `Please provide your ${formPayment === "gcash" ? "GCash" : "Maya"} number`,
      );
      return;
    }
    try {
  ...
  const handleDeletePost = async () => {
    try {
      ...
      setActivePosts((prev) =>
        prev.filter((job) => job.id !== showDeleteModal.id),
      );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 shrink-0">
        <div>
          ...
          <p className="text-brand-text-variant font-medium">
            Manage your public listings and review professional bids.
          </p>
        </div>
          ...
          <div className="py-12 flex flex-col items-center justify-center">
            <div className="w-full max-w-4xl bg-brand-surface-card border-2 border-brand-outline rounded-[3rem] p-12 md:p-20 relative overflow-hidden shadow-2xl">
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <h2 className="text-4xl md:text-5xl font-black text-brand-text-main mb-6 leading-tight">
                  Ready to get things done? 🛠️
                </h2>
                <p className="text-brand-text-variant text-xl max-w-2xl mb-12 leading-relaxed">
                  Your marketplace is currently quiet. Start a new project today
                  and let LingkodHub's professionals handle the rest.
                </p>

                <div className="w-full max-w-2xl">
                  <div className="flex items-center justify-center gap-4 mb-8">
                    <div className="h-px flex-1 bg-brand-outline"></div>
                    <span className="text-xs font-black text-brand-text-variant uppercase tracking-[0.2em]">
            ...
            {activePosts.map((job) => (
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <div
                        className={`w-2 h-2 rounded-full ${job.applicants.length > 0 ? "bg-brand-primary animate-ping" : "bg-brand-text-variant/30"}`}
                      ></div>
                      <span className="text-[10px] font-black text-brand-text-variant uppercase tracking-widest">
                        {job.provider_id
                          ...
                          : job.applicants.length === 0
                            ? "Awaiting Bids"
                            : `${job.applicants.length} Active Applicants`}
                      </span>
                    </div>
                  </div>
                  <button
                ...
                <div className="mb-6 flex-1">
                  <h3
                    className={`text-2xl font-black leading-tight mb-4 group-hover:text-brand-primary transition-colors ${job.is_urgent ? "text-orange-600" : "text-brand-text-main"}`}
                  >
                    {job.title}
                  </h3>
                ...
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-[0.2em] mb-1">
                      Project Budget
                    </p>
                    <div className="flex items-baseline gap-1">
                      <span className="text-2xl font-black text-brand-text-main">
                    ...
                    {job.applicants.slice(0, 3).map((app, i) =>
                      app.avatar_url ? (
                        <img
                          key={i}
                          src={
                            app.avatar_url.startsWith("http")
                              ? app.avatar_url
                              : `http://localhost:3000${app.avatar_url}`
                          }
                ...
                <div className="pt-6 border-t border-brand-outline/50 flex flex-col gap-4">
                  <button
                    ...
                    disabled={
                      job.provider_id ? true : job.applicants.length === 0
                    }
                  ...
                  >
                    {job.provider_id
                      ...
                      : `View All Bids ${job.applicants.length > 0 ? `(${job.applicants.length})` : ""}`}
          ...
          <div className="relative bg-brand-surface-card w-full max-w-5xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 flex flex-col text-left max-h-[95vh] border border-white/10">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-black text-brand-text-main tracking-tight uppercase">
                    {invitedProvider
                      ? `Inviting ${invitedProvider.full_name}`
                      : "Create a New Task"}
                  </h3>
                  <p className="text-sm font-medium text-brand-text-variant">
                    Fill in the logistics and secure your professional.
                  </p>
                </div>
              </div>
              <button
                onClick={() => setShowPostModal(false)}
                className="p-3 bg-brand-surface border border-brand-outline text-brand-text-variant hover:text-brand-text-main rounded-2xl transition-all shadow-sm"
              >
                <X size={20} />
              </button>
            </div>
                ...
                <div className="space-y-10">
                  <div className="flex items-center gap-3 border-b border-brand-outline pb-4">
                    ...
                    <h4 className="text-sm font-black text-brand-text-main uppercase tracking-widest">
                      Section 1: Scope & Detail
                    </h4>
                  </div>
                  <div className="space-y-8">
                    <div>
                      <label className="block text-[10px] font-black text-brand-text-variant uppercase tracking-[0.2em] mb-3 ml-1">
                        What do you need help with?
                      </label>
                      <input
                        value={formTitle}
                        onChange={(e) => setFormTitle(e.target.value)}
                        placeholder="e.g., Deep Cleaning for 3BR Apartment"
                        className="w-full p-5 rounded-[1.5rem] border-2 border-brand-outline focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5 transition-all text-sm font-bold text-brand-text-main bg-brand-surface outline-none"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full p-5 rounded-[1.5rem] border-2 border-brand-outline focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5 transition-all text-sm font-bold text-brand-text-main bg-brand-surface outline-none appearance-none cursor-pointer"
                        >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="500"
                          className="w-full p-5 rounded-[1.5rem] border-2 border-brand-outline focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5 transition-all text-lg font-black text-brand-text-main bg-brand-surface outline-none"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="The more detail you provide, the better bids you'll receive..."
                        className="w-full p-5 rounded-[1.5rem] border-2 border-brand-outline focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5 transition-all text-sm font-bold text-brand-text-main bg-brand-surface outline-none h-44 resize-none leading-relaxed"
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-8">
                      ...
                      <div className="space-y-4">
                        {addresses.length > 0 && (
                          <div className="relative group">
                            ...
                            <ChevronDown
                              className="absolute right-5 top-1/2 -translate-y-1/2 text-brand-text-variant group-hover:text-brand-primary pointer-events-none"
                              size={20}
                            />
                          </div>
                        )}
                        {(selectedAddressId === "new" ||
                          ...
                          <div className="relative animate-in slide-in-from-top-2 duration-300">
                            <MapPin
                              className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-text-variant"
                              size={20}
                            />
                            <input
                              value={formLocation}
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="relative group">
                          <Calendar
                            className={`absolute left-5 top-1/2 -translate-y-1/2 ${formIsASAP ? "text-gray-300" : "text-brand-text-variant group-focus-within:text-brand-primary"} transition-colors`}
                            size={20}
                          />
                          <input
                            type="datetime-local"
                      ...
                      <div className="flex items-end">
                        ...
                        >
                          <Zap
                            size={20}
                            className={formIsASAP ? "animate-pulse" : ""}
                          />
                    ...
                    {/* Payment Setup */}
                    <div>
                      <label className="block text-[10px] font-black text-brand-text-variant uppercase tracking-[0.2em] mb-4 ml-1">
                        Payout Method
                      </label>
                      ...
                      <div className="relative animate-in fade-in duration-500">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            formPayment === "gcash"
                              ? setTempGcashNumber(e.target.value)
                              : setTempMayaNumber(e.target.value)
                          }
                    ...
                    <label className="flex items-center gap-3 p-4 bg-brand-surface border border-brand-outline rounded-2xl cursor-pointer hover:bg-brand-surface-card transition-colors group">
                      <div
                        className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${saveDetails ? "bg-brand-primary border-brand-primary shadow-lg shadow-brand-primary/20" : "border-brand-outline group-hover:border-brand-primary"}`}
                      >
                        {saveDetails && (
                      ...
                      <span className="text-xs font-bold text-brand-text-main">
                        Save these details to my profile for future tasks
                      </span>
                      <input
                        type="checkbox"
                        checked={saveDetails}
            ...
            <div className="p-10 border-t border-brand-outline bg-brand-surface/80 backdrop-blur-lg flex justify-between items-center sticky bottom-0 z-10">
              <div className="hidden sm:flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-brand-text-variant uppercase tracking-[0.2em] mb-0.5">
                    Total Commitment
                  </p>
                  <p className="text-2xl font-black text-brand-text-main">
                    ₱{formBudget ? parseInt(formBudget).toLocaleString() : "0"}
                  </p>
                </div>
              </div>
              <div className="flex gap-4 w-full sm:w-auto">
                <button
                  onClick={() => setShowPostModal(false)}
                  className="px-8 py-5 font-black text-brand-text-variant hover:text-brand-text-main transition-all uppercase tracking-[0.2em] text-[10px]"
                >
                  Cancel
                </button>
                <button
                  onClick={handlePostJob}
                  className="px-12 py-5 bg-brand-primary text-white rounded-[1.5rem] font-black text-sm hover:bg-[#047857] hover:scale-[1.05] shadow-2xl shadow-brand-primary/40 transition-all active:scale-95 flex items-center gap-3 uppercase tracking-wider"
                >
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-brand-surface-card w-full max-w-md rounded-[2.5rem] shadow-2xl p-10 text-center animate-in fade-in zoom-in-95 duration-200 border border-white/10">
            ...
            <h3 className="text-2xl font-black text-brand-text-main mb-4 tracking-tight uppercase">
              Revoke Listing?
            </h3>
            <p className="text-brand-text-variant mb-10 font-medium leading-relaxed">
              This will permanently remove{" "}
              <strong>"{showDeleteModal.title}"</strong> and dismiss all current
              bids.
            </p>
            <div className="flex gap-4">
              <button
                onClick={() => setShowDeleteModal(null)}
                className="flex-1 py-4 bg-brand-surface border border-brand-outline rounded-2xl font-black text-xs uppercase tracking-widest text-brand-text-main hover:bg-brand-surface-card transition-all"
              >
                Dismiss
              </button>
              <button
                onClick={handleDeletePost}
                className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-red-500/20 hover:bg-red-700 transition-all"
              >
                Delete Post
              </button>
            </div>
          ...
          <div className="relative bg-brand-surface-card w-full max-w-3xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 flex flex-col text-left max-h-[85vh] border border-white/10">
            <div className="p-8 border-b border-brand-outline flex justify-between items-center bg-brand-surface/50 backdrop-blur-md sticky top-0 z-10">
              <div>
                ...
                <p className="text-sm font-medium text-brand-text-variant mt-1">
                  Select the best professional for your task.
                </p>
              </div>
              <button
                onClick={() => setViewApplicantsJob(null)}
            ...
            <div className="p-8 overflow-y-auto bg-brand-surface/30 space-y-4">
              {viewApplicantsJob.applicants.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-brand-text-main font-bold">
                    No bids received yet.
                  </p>
                  <p className="text-sm text-brand-text-variant mt-2">
                    Professionals will appear here once they apply.
                  </p>
                </div>
              ) : (
                viewApplicantsJob.applicants.map((app) => (
                  <div
                    key={app.id}
                    className="bg-brand-surface border-2 border-brand-outline rounded-3xl p-6 flex flex-col sm:flex-row gap-6 items-start sm:items-center hover:border-brand-primary/50 transition-all hover:shadow-lg"
                  >
                    {app.avatar_url ? (
                      <img
                        src={
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="text-lg font-black text-brand-text-main">
                          {app.full_name || "Professional"}
                        </h4>
                        <span className="bg-brand-primary/10 text-brand-primary text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider">
                          Verified
                        </span>
                      </div>
                      <p className="text-sm font-medium text-brand-text-variant mb-4 bg-brand-surface-card p-3 rounded-xl border border-brand-outline/50">
                        "
                        {app.message ||
                          "I am interested in this task and available to help."}
                        "
                      </p>
                      <div className="flex flex-wrap items-center gap-4 text-[10px] font-black uppercase tracking-widest text-brand-text-variant">
                        <span className="flex items-center gap-1.5">
                          <Star size={14} className="text-amber-500" /> 4.9
                          Rating
                        </span>
                        <span className="flex items-center gap-1.5">
                          <MapPin size={14} className="text-blue-500" />{" "}
                          {app.location || "Local Provider"}
                        </span>
                      </div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                      <button
                        onClick={async () => {
                          if (
                            !confirm(
                              `Decline application from ${app.full_name}?`,
                            )
                          )
                            return;
                          try {
