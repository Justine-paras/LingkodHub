...
router.put(
  ...
  (req, res) => {
    if (req.user.role !== "client") {
      res
        .status(403)
        .json({ error: "Only clients can decide on applications." });
      return;
    }
  ...
  `,
    ...
    if (application.job_status !== "pending") {
      res
        .status(400)
        .json({ error: "This job is no longer accepting decisions." });
      return;
    }
    ...
    if (status === "accepted") {
      db.transaction(() => {
        ...
        db.prepare(
          "UPDATE applications SET status = ? WHERE job_id = ? AND id != ?",
        ).run("rejected", application.job_id, application.id);
        const finalPaymentMethod = payment_method || "gcash";
        db.prepare(
          "UPDATE jobs SET status = ?, provider_id = ?, payment_method = ? WHERE id = ?",
        ).run(
          "pending",
      ...
      notify(
        ...
        `Your application for "${application.job_title}" was accepted.`,
        application.job_id,
      );
      ...
      const rejected = db
        ...
        .all(application.job_id, "rejected", application.id);
      rejected.forEach((r) =>
        notify(
          r.provider_id,
          "application_rejected",
          "Application Not Selected",
          `Another provider was selected for "${application.job_title}".`,
          application.job_id,
        ),
      );
    } else {
...
router.delete("/:id", authenticateToken, (req, res) => {
  if (req.user.role !== "provider") {
    res
      .status(403)
      .json({ error: "Only providers can withdraw applications." });
    return;
  }
  ...
  if (application.status !== "pending") {
    res
      ...
      .json({
        error:
          "Cannot withdraw an application that is already accepted or rejected.",
      });
    return;
  }
