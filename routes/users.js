...
router.put("/me", authenticateToken, validate(updateMeSchema), (req, res) => {
  ...
  if (pref_email_messages !== undefined) {
    updates.push("pref_email_messages = ?");
    params.push(pref_email_messages);
  }
  if (pref_email_updates !== undefined) {
    updates.push("pref_email_updates = ?");
    params.push(pref_email_updates);
  }
  if (pref_email_promos !== undefined) {
    updates.push("pref_email_promos = ?");
    params.push(pref_email_promos);
  }
  if (pref_push_alerts !== undefined) {
    updates.push("pref_push_alerts = ?");
    params.push(pref_push_alerts);
  }
  if (pref_push_marketing !== undefined) {
    updates.push("pref_push_marketing = ?");
    params.push(pref_push_marketing);
  }
  if (is_public_profile !== undefined) {
    updates.push("is_public_profile = ?");
    params.push(is_public_profile);
  }
  if (show_online_status !== undefined) {
    updates.push("show_online_status = ?");
    params.push(show_online_status);
  }
  ...
  if (maya_number !== undefined) {
    updates.push("maya_number = ?");
    params.push(maya_number);
  }
...
router.get("/me/billing", authenticateToken, (req, res) => {
  const payments = db
    .prepare(
      `
...
router.get("/me/services", authenticateToken, (req, res) => {
  if (req.user.role !== "provider") {
    res.status(403).json({ error: "Only providers have services." });
    return;
  }
  res.json(
    db
...
router.post(
  ...
  (req, res) => {
    ...
    db.prepare("DELETE FROM provider_services WHERE provider_id = ?").run(
      req.user.id,
    );
    ...
    if (services.length > 0) {
      ...
      db.transaction(() => {
        for (const name of services) {
          addService.run(name);
          const svc = getServiceId.get(name);
...
router.delete("/me", authenticateToken, (req, res) => {
  try {
    db.transaction(() => {
      ...
      db.prepare(
        "UPDATE jobs SET provider_id = NULL WHERE provider_id = ?",
      ).run(req.user.id);
    ...
    res
      .status(500)
      .json({
        error: "Failed to delete account due to a database constraint.",
      });
  }
});
...
router.get("/me/applications", authenticateToken, (req, res) => {
  if (req.user.role !== "provider") {
    res.status(403).json({ error: "Only providers have applications." });
    return;
  }
  res.json(
    db
...
router.post(
  ...
  (req, res) => {
    ...
    const currentUser = db
      .prepare(
        "SELECT verification_document_url, verification_selfie_url FROM users WHERE id = ?",
      )
      .get(req.user.id);

    const docUrl = docFile
      ? `/uploads/${docFile.filename}`
      : currentUser?.verification_document_url;
    const selfieUrl = selfieFile
      ? `/uploads/${selfieFile.filename}`
      : currentUser?.verification_selfie_url;
    ...
    db.prepare(
      "UPDATE users SET verification_document_url = ?, verification_selfie_url = ?, is_documents_verified = ?, document_status = ? WHERE id = ?",
    ...
    if (isVerified) {
      db.prepare(
        ...
        req.user.id,
        "Identity Verified",
        "Congratulations! Your document and selfie have been verified successfully. You are now a System Verified Provider.",
        "success",
      );
    } else {
      db.prepare(
...
router.get("/me/addresses", authenticateToken, (req, res) => {
  const addresses = db
    .prepare(
...
router.post(
  ...
  (req, res) => {
    ...
    const count = db
      .prepare("SELECT COUNT(*) as count FROM user_addresses WHERE user_id = ?")
      .get(req.user.id);
    if (count.count >= 3) {
      res
        .status(400)
        .json({ error: "You can only have a maximum of 3 saved addresses." });
      return;
    }
    ...
    db.transaction(() => {
      ...
      db.prepare(
        "INSERT INTO user_addresses (user_id, label, address_text, is_default) VALUES (?, ?, ?, ?)",
...
router.delete("/me/addresses/:id", authenticateToken, (req, res) => {
  const { id } = req.params;
  db.prepare("DELETE FROM user_addresses WHERE id = ? AND user_id = ?").run(
    id,
    req.user.id,
  );
  res.json({ success: true });
...
router.patch("/me/addresses/:id/default", authenticateToken, (req, res) => {
  const { id } = req.params;
  db.transaction(() => {
    db.prepare(
      "UPDATE user_addresses SET is_default = 0 WHERE user_id = ?",
