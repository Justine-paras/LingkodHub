...
function issueAuthCookies(res, token, refreshToken) {
...
function issueRefreshToken(userId) {
  ...
  const token = jwt.sign({ id: userId, jti }, process.env.JWT_SECRET, {
    expiresIn: "7d",
  });
...
function revokeRefreshToken(token, replacedByToken) {
...
router.post("/refresh", (req, res) => {
  const refreshToken =
    parseCookie(req, "refresh_token") || req.body?.refreshToken;
  if (!refreshToken) {
    res.sendStatus(401);
    return;
  }
...
router.post(
  ...
  (req, res) => {
    ...
    if (
      !user ||
      !bcrypt.compareSync(req.body.currentPassword, user.password_hash)
    ) {
...
router.post("/send-otp", authenticateToken, async (req, res) => {
  ...
  db.prepare("UPDATE users SET otp = ?, otp_expires_at = ? WHERE id = ?").run(
    otp,
    expiresAt,
    req.user.id,
  );
...
router.post("/verify-otp", authenticateToken, (req, res) => {
  const { otp } = req.body;
  const user = db
    .prepare("SELECT otp, otp_expires_at FROM users WHERE id = ?")
    .get(req.user.id);
  ...
  db.prepare(
    "UPDATE users SET is_email_verified = 1, otp = NULL, otp_expires_at = NULL WHERE id = ?",
  ).run(req.user.id);
