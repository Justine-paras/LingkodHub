...
router.get('/:userId', authenticateToken, (req: AuthRequest, res: Response) => {
  const me    = req.user!.id;
  const other = Number(req.params.userId);
  if (isNaN(other)) { res.status(400).json({ error: 'Invalid user ID.' }); return; }
...
router.post('/', authenticateToken, validate(sendMessageSchema), (req: AuthRequest, res: Response) => {
  ...
  if (receiver_id === req.user!.id) { res.status(400).json({ error: 'Cannot message yourself.' }); return; }
  ...
  notify(receiver_id, 'new_message', 'New Message',
    `${sender?.full_name} sent you a message: "${preview}"`, req.user!.id);

  res.status(201).json(db.prepare('SELECT * FROM messages WHERE id = ?').get(result.lastInsertRowid));
});

// Mark all messages from a sender as read
router.put('/read/:userId', authenticateToken, (req: AuthRequest, res: Response) => {
  const other = Number(req.params.userId);
  if (isNaN(other)) { res.status(400).json({ error: 'Invalid user ID.' }); return; }
