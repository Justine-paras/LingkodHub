...
export async function sendEmail({ to, subject, html }) {
  ...
  if (!emailUser || !emailPass) {
    console.warn(
      "[Email Service] EMAIL_USER or EMAIL_PASS not set. Logging email instead:",
    );
  ...
  const mailOptions = {
    from: `"LingkodHub Support" <${emailUser}>`,
    to,
    subject,
    html,
  };
  ...
  try {
    ...
    console.warn(
      `[Email Service] FALLBACK: OTP for ${to} is being logged because email failed.`,
    );
    console.log(
      `[OTP FALLBACK] To: ${to}\nSubject: ${subject}\nContent: ${html}`,
    );
