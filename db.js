...
export function initDb() {
  ...
  if (!jobCols.includes("provider_id")) {
    db.exec(
      "ALTER TABLE jobs ADD COLUMN provider_id INTEGER REFERENCES users(id)",
    );
  ...
  if (!userCols.includes("is_documents_verified")) {
    db.exec(
      "ALTER TABLE users ADD COLUMN is_documents_verified INTEGER DEFAULT 0",
    );
  ...
  if (!userCols.includes("pref_email_messages")) {
    ...
    db.exec(
      "ALTER TABLE users ADD COLUMN pref_email_updates INTEGER DEFAULT 1",
    );
    ...
    db.exec(
      "ALTER TABLE users ADD COLUMN pref_push_marketing INTEGER DEFAULT 0",
    );
  ...
  if (!userCols.includes("is_public_profile")) {
    ...
    db.exec(
      "ALTER TABLE users ADD COLUMN show_online_status INTEGER DEFAULT 0",
    );
...
export function notify(userId, type, title, body, referenceId) {
  db.prepare(
